...
(async () => {
  ...
  if (isFileProtocol) {
    console.error('[loader] Running on file://. Partials cannot be fetched reliably without an HTTP server (use Live Server or "npx serve").');

    const first = slots[0];
    if (first) {
      first.insertAdjacentHTML('beforebegin',
        '<p style="color:#b91c1c;font-family:monospace;padding:1rem;margin:0">[loader] Open this site via http:// (Live Server) instead of file:// to load partial sections.</p>');
    }
  ...
  await Promise.all(slots.map(async slot => {
    ...
    try {
      ...
      // Leave a visible marker only on localhost dev.
      if (isLocalDev) {
